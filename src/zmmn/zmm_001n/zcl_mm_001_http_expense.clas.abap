CLASS zcl_mm_001_http_expense DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:   items TYPE STANDARD TABLE OF zmm_001_s_expense_item WITH EMPTY KEY,
           ...
           BEGIN OF ty_output,
             items TYPE STANDARD TABLE OF zmm_001_s_expense_response WITH EMPTY KEY,
           END OF ty_output.
  ...
  PRIVATE SECTION.
    DATA: ls_req            TYPE ty_inputs,
          ...
          lt_return_inv     TYPE STANDARD TABLE OF ty_return WITH EMPTY KEY,
          lt_return_po      TYPE STANDARD TABLE OF ty_return WITH EMPTY KEY.
ENDCLASS.



CLASS zcl_mm_001_http_expense IMPLEMENTATION.
  ...
  METHOD if_http_service_extension~handle_request.
    ...
    DATA(lo_run) = NEW zmm_001_cl_expense( input = ls_req ).
...
*    ).
    ...
    IF lv_po_number IS NOT INITIAL.
      ...
      lo_run->create_invoice(
        ...
        CHANGING
          ct_return         = lt_return_inv
      ).

    ENDIF.

    IF lt_return_po IS NOT INITIAL.
      APPEND LINES OF lt_return_po TO lt_return_inv.
    ENDIF.
...
*      <lfs_return>-message = |İşlem başarısız|.
    ...
    DATA(lv_json_string) = xco_cp_json=>data->from_abap( lt_return_inv )->apply( VALUE #(
